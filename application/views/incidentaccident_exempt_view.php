...
<body>
                ...
                <?php $i=0; foreach($all_exemptincident as $row): ?>
                    <tr class="searchable">
                        ...
                        <td><?php 
                            ...
                            ?></td>
                        <td><button type="button" name = 'load' value = 'Load' onclick="location.href='<?php echo site_url().'/incidentaccidentreportingpageexemptproj/load_project?id='.$row->project_id;?>'" class="btn btn-primary">Load</button></td>
    ...
    <script>
        ...
        function reject2(i,k){
            var j = prompt("Reason for Rejecting:", "Did not meet requirement");
            if (j != null) {
                window.location = "<?php echo base_url(); ?>index.php/incidentaccident_exempt/reject2/" + i + "/" + k + "/" + btoa(j);
            }
        }
    </script>
</body>
